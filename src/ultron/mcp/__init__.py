...
def _lifecycle_not_configured(operation: str) -> Never:
    raise RuntimeError(
        f"{operation} não foi executada: o adapter de lifecycle do consumer não está configurado"
    )
...
async def installation_list(consumer_id: str) -> dict[str, Any]:
    """Consulta instalações; falha de forma explícita sem adapter do consumer."""
    if not consumer_id.strip():
        raise ValueError("consumer_id não pode ser vazio")
    _lifecycle_not_configured("installation.list")


@mcp.tool(name="ultron_capability_status")
async def capability_status(capability_id: str, consumer_id: str) -> dict[str, Any]:
    """Consulta estado de uma capability no consumer indicado."""
    if not capability_id.strip() or not consumer_id.strip():
        raise ValueError("capability_id e consumer_id são obrigatórios")
    _lifecycle_not_configured("capability.status")
...
async def capability_install(
    capability_id: str, consumer_id: str, confirmed: bool = False
) -> dict[str, Any]:
    """Solicita plano de instalação após confirmação explícita."""
    _require_confirmation(confirmed, "capability.install")
    _lifecycle_not_configured("capability.install")
...
async def capability_activate(
    ...
    _lifecycle_not_configured("capability.activate")
...
async def capability_deactivate(
    ...
    _lifecycle_not_configured("capability.deactivate")
...
async def capability_remove(
    ...
    _lifecycle_not_configured("capability.remove")
